...
class TestCollaborationService:
    ...
    def test_is_admin_true(self):
        with patch("src.Service.CollaborationService.CollaborationDAO") as MockCollabDAO, \
    ...
    def test_is_viewer_true(self):
        with patch("src.Service.CollaborationService.CollaborationDAO") as MockCollabDAO, \
            ...
            mock_dao = MagicMock()
            MockCollabDAO.return_value = mock_dao
            mock_dao.find_by_conversation_and_user.return_value = Collaboration(1, 10, 100, "viewer")
    ...
    def test_create_collab_already_exists(self):
        with patch("src.Service.CollaborationService.CollaborationDAO") as MockCollabDAO, \
            ...
            mock_collab_dao.find_by_conversation_and_user.return_value = Collaboration(1, 10, 100, "writer")
    ...
    def test_verify_token_collaboration_success(self):
        with patch("src.Service.CollaborationService.CollaborationDAO"), \
             patch("src.Service.CollaborationService.UserDAO"), \
             patch("src.Service.CollaborationService.ConversationDAO") as MockConvDAO:

            mock_conv_dao = MagicMock()
            MockConvDAO.return_value = mock_conv_dao
            mock_conv_dao.read.return_value = MagicMock(token_viewer="token123", token_writter="token456")
